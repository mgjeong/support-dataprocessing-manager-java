...
//        this.jobTableManager = mock(JobTableManager.class);
        ...
        when(EngineFactory.createEngine(EngineType.Flink, "localhost", 8081)).thenReturn(framework);
    ...
    public void eCrudJobTest() throws Exception {
        ...
        job.setTargetHost("localhost:8082");
        job.setRuntimeHost("localhost:8082");
        job.addInput(input);
        job.addOutput(output);
        job.addTask(task);
        jobGroup.addJob(job);

        jobManager.createGroupJob(EngineType.Flink, jobGroup);

        jobGroupResponse = jobManager.getAllJobs();
        Assert.assertEquals(jobGroupResponse.getJobGroups().size(), 3);

        jobManager.updateJob(EngineType.Flink, "ef91c5b3-b5bb-4e8b-8145-e271ac16ce30", jobGroup);
        jobGroupResponse = jobManager.getAllJobs();
        Assert.assertEquals(jobGroupResponse.getJobGroups().size(), 3);
        ...
        jobManager.updateJob(EngineType.Flink, "ef91c5b3-b5bb-4e8b-8145-e271ac16ce40", jobGroup);
        jobGroupResponse = jobManager.getAllJobs();
...
//        jobManager.updateJob("ef91c5b3-b5bb-4e8b-8145-e271ac16cef1", jobGroup);
    ...
    private static List<Map<String, String>> makeMockPayload() {
        ...
        p.put(JobTableManager.Entry.engineType.name(), "Flink");
        p.put(JobTableManager.Entry.runtimeHost.name(), "localhost:8082");
        p.put(JobTableManager.Entry.targetHost.name(), "localhost:8082");
        // invalid param
