...
import org.edgexfoundry.processing.runtime.task.model.AutoregressiveMovingAverage;
import org.edgexfoundry.processing.runtime.task.model.ExponentialMovingAverage;
import org.edgexfoundry.processing.runtime.task.model.MovingAverage;
import org.edgexfoundry.processing.runtime.task.model.SimpleMovingAverage;
import org.edgexfoundry.processing.runtime.task.model.WeightedMovingAverage;
...
public class MovingAverageTest {
    ...
    public void testGetProcessingType() {
        MovingAverage model = new SimpleMovingAverage();
        Assert.assertEquals(TaskType.TREND, model.getType());
    }

    @Test
    public void testGetName() {
        MovingAverage model1 = new SimpleMovingAverage();
        Assert.assertEquals("sma", model1.getName());

        MovingAverage model2 = new AutoregressiveMovingAverage();
        Assert.assertEquals("arma", model2.getName());

        MovingAverage model3 = new ExponentialMovingAverage();
        Assert.assertEquals("ema", model3.getName());

        MovingAverage model4 = new WeightedMovingAverage();
        Assert.assertEquals("wma", model4.getName());
    }
    ...
    public void testCalculate() {
        ...
        String logMessage = "======== SIMPLE MOVING AVERAGE BASED == Window Type: ";
        MovingAverage model = new SimpleMovingAverage();
        TaskModelParam base = new TaskModelParam();
        TaskModelParam data = new TaskModelParam();
        data.put("data", 3);
        ...
        logMessage = "======== EXPONENTIAL MOVING AVERAGE BASED ========";
        MovingAverage model1 = new ExponentialMovingAverage();
        base.clear();
        data.clear();
        data.put("data", 5);
        base.put("interval", data);
        base.put("target", target);
        base.put("outputKey", output);
        model1.setParam(base);
        System.out.println(logMessage);
        for (int i = 0; i < records.length; i++) {
            //for (int i = 0; i < 2; i++) {
            DataSet dataSet = DataSet.create();
            dataSet.addRecord(records[i]);
            dataSet = model1.calculate(dataSet, null, null);
            if (dataSet == null) {
                System.out.println("ignored input " + i);
            } else {
        ...
        logMessage = "======== WEIGHTED MOVING AVERAGE BASED ========";
        MovingAverage model2 = new WeightedMovingAverage();

        Double[] weightsArray = {0.8, 0.15, 0.05};
        ArrayList<Double> weights =
                new ArrayList<Double>(Arrays.asList(weightsArray));
        ...
        base.put("target", target);
        base.put("outputKey", output);
        TaskModelParam ma = new TaskModelParam();
        ma.put("coefficients", weights);
        base.put("ma", ma);
        model2.setParam(base);
        System.out.println(logMessage);
        for (int i = 0; i < records.length; i++) {
            //for (int i = 0; i < 2; i++) {
            DataSet dataSet = DataSet.create();
            dataSet.addRecord(records[i]);
            dataSet = model2.calculate(dataSet, null, null);
            if (dataSet == null) {
                System.out.println("ignored input " + i);
            } else {
        ...
        logMessage = "======== AUTO REGRESSIVE MOVING AVERAGE BASED ========";
        MovingAverage model3 = new AutoregressiveMovingAverage();

        Double[] maCoefficientsArray = {0.8, 0.15, 0.05};
        Double[] arCoefficientsArray = {0.001, 0.001, 0.001, 0.0005, 0.0};
        Double[] lagOperatorsArray = {0.001, 0.001, 0.001, 0.0001, 0.0};
        ...
        base.put("ma", ma);
        TaskModelParam ar = new TaskModelParam();
        ar.put("coefficients", arCoefficients);
        base.put("ar", ar);
        TaskModelParam lags = new TaskModelParam();
        lags.put("coefficients", lagOperators);
        base.put("lags", lags);
        model3.setParam(base);
        ...
        for (int i = 0; i < records.length; i++) {
            ...
            DataSet dataSet = DataSet.create();
            dataSet.addRecord(records[i]);
            dataSet = model3.calculate(dataSet, null, null);
            if (dataSet == null) {
                System.out.println("ignored input " + i);
            } else {
